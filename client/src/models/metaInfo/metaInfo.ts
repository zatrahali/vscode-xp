...
export class MetaInfo {
  ...
  public static create(metaInfoAsInFile: any): MetaInfo {
    ...
    if (attackDict) {
      metaInfo.ATTACK = Object.keys(attackDict).map((tactic) => {
        const ta = new Attack();
        ta.Tactic = tactic;
  ...
  public toString(): string {
    ...
    if (this.EventDescriptions.length != 0) {
      metaInfoObject['EventDescriptions'] = this.EventDescriptions.map(function (ed, index) {
  ...
  public correctEventIds(metaInfoContent: string): string {
    ...
    while ((curResult = eventIdItemRegExp.exec(metaInfoContent))) {
      if (curResult.length != 2) continue;

...
export abstract class RuleBaseItem extends ContentTreeBaseItem {
  ...
  public getPackagePath(config: Configuration): string {
    ...
    const roots = config.getContentRoots().map((folder) => {
    ...
    throw new XpException(
      `Путь к правилу ${this.getName()} не содержит ни одну из корневых директорий: [${roots.join(', ')}]. Приведите структуру в соответствие ([пример](https://github.com/Security-Experts-Community/open-xp-rules)) и повторите`
    );
  }
  ...
  public reloadIntegrationTests(): void {
    const reloadedIntegrationTests = IntegrationTest.parseFromRuleDirectory(
      this.getDirectoryPath()
    );

    for (
      let reloadedTestIndex = 0;
      reloadedTestIndex < reloadedIntegrationTests.length;
      reloadedTestIndex++
    ) {
  ...
  public async saveIntegrationTests(ruleDirPath?: string): Promise<void> {
    ...
    if (fs.existsSync(testDirectoryPath)) {
      ...
      const rawEventFilePaths = FileSystemHelper.readFilesNameFilter(
        testDirectoryPath,
        /raw_events_\d+.json/g
      ).map((fileName) => path.join(testDirectoryPath, fileName));
  ...
  protected async saveLocalization(fullPath: string): Promise<void> {
    ...
    const ruEventDescriptions = this._localizations.map(function (loc) {
      ...
      if (!locId) {
        throw new XpException(
          `Ошибка целостности локализаций правила ${fullPath}, не задан localizationId`
        );
      }
  ...
  protected checkLocalizationConsistency(

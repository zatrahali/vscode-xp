...
export class FunctionsLocalePathLocator extends BaseLocaleFileLocator {
    constructor(currentLanguageTag: string, private extensionDirPath: string) {
		super(currentLanguageTag);
	}

	protected getLocaleFilePathImpl(tag: string) : string {
		const localeFilePath = path.join(this.extensionDirPath, LocalizationService.LOCALIZATION_DIRNAME, `co.functions.${tag}.json`);

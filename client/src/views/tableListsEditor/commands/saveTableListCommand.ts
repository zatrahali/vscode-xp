...
export class SaveTableListCommand implements TableListCommand {
  public processMessage(message: TableListMessage): void {
    ...
    if (!message.payload) {
      throw new XpException(
        `В команду ${SaveTableListCommand.commandName} не было передано поле 'payload'`
      );
    }

    const jsonTableView = message.payload;
    const tableObject = jsonTableView;
    ...
    if (!tableObject.metainfo.ruDescription && !tableObject.metainfo.enDescription) {
      throw new XpException(
        `Не задано описание табличного списка. Задайте хотя бы описание на одном языке и повторите`
      );
    }

    if (
      tableObject.typicalSize &&
      tableObject.maxSize &&
      tableObject.typicalSize >= tableObject.maxSize
    ) {
      throw new XpException(
        `Не может типичный размер табличного списка быть больше или равен максимальному. Исправьте данные параметры и повторите`
      );
    }
    ...
    if (duplicateColumnName) {
      throw new XpException(
        `Имена колонок должны быть уникальными в рамках табличного списка. Колонка ${duplicateColumnName} дублируется`
      );
    }
  ...
  async execute(webView: TableListsEditorViewProvider): Promise<boolean> {
    ...
    const tableObject = this._message.payload;
    ...
    if (webView.getTable()) {
      ...
      this._newTable = Table.create(
        tableObject.name,
        tableObject.fillType,
        parentItem.getDirectoryPath()
      );
    }
  ...
  static get commandName(): string {
    return 'TableListEditor.saveTableList';
  }
}
